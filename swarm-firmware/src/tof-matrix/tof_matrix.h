...
tof_err_code_e config_sensors(VL53L5CX_Configuration *p_dev, uint16_t new_i2c_address);
tof_err_code_e get_tof_matrix(uint8_t sensor_id, int16_t matrix[][8]);
tof_err_code_e get_tof_row(uint8_t sensor_id, uint8_t rowIdx, int16_t* row); // row 0 is the top one
